...
class Drug_Dataset(Dataset):
    ...
    def __reorder__(self, non_test_idx=None):
        ...
        if non_test_idx is None:
            temp_mat = np.append(self.input_data, self.target_data.T, axis=0)
        else:
            temp_mat = np.append(self.input_data[:, non_test_idx],
                                  self.target_data[non_test_idx].T, axis=0)
    
        temp1 = np.corrcoef(temp_mat)
        shape = temp1.shape[0]
        corr_list = temp1[shape-1, :shape-1]
        ...
        for i in range(0, 10000):
            F_old = F  
            F = np.dot(.95*F_old,S) + (1-.95)*Y
            if(np.amax(abs(np.subtract(F,F_old))) < 1e-8):
                break
        if (i==9999):
            print("No converge")

        corr_index = F.argsort()[::-1]
...
def train(model, criterion, optimizer, train_loader, epoch,learning_rate, reduction='avg',
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        output = model(torch.cat((data[:, 0:rank//2],data[:,-rank//2:]),axis=1))
...
N_input = 100 #np.size(expr_data,0)  # rank
N_target = 1
...
np.savetxt('corr_nn_2_base_gb_100_6631.csv',np.transpose(output),fmt='%s',delimiter=',')  
